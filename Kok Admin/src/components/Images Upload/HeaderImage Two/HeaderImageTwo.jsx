...
const HeaderImageTwo = () => {
  ...
  const handleUploadImage = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_BASE_URL}/Admin/headerimagetwoInsUp`,
        formData,
        {
          headers: {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/Admin/headerimagetwoFill`,
        {
          headers: {
  ...
  return (
    <>
      <Title level={2}>Header Image Two Upload</Title>
      <Divider />
      <div>
        <Upload
